...
//    			])->find();
            ...
            $d_user['token']= gen_jwt_token(['user_id'=> $d_user['id'], 'openid'=> $open_id, 'session_key'=> $session_key]);
            $response=$d_user;
//			$response = [
//				'user_info'=> $d_user,
...
//            ])->find();
     ...
     * @param VUser $v_user
     */
    public function password(MAppUser $m_user, VUser $v_user)
    {
        try {
            $param = $this->request->param();
            ...
            if($d_user["password"] != $password){
                return $this->api_data('DATA_NOT_EXIST', '', '手机号或密码不正确');
            }
            $session_key = 'session_key';
            $d_user['token']=gen_jwt_token([
                'user_id'=> $d_user['id'],
                'openid'=> $d_user["openid"],
                'session_key'=> $session_key
            ]);
